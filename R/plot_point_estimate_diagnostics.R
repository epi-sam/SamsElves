
#' Plots histogram of the percentile of the `point_estimate` relative to the
#' draws and scatters of `point_estimate` versus either the `mean` or `median`
#' of the draws.
#'
#' @param DT [data.table] summarized draws from `get_draw_pe_ui_difference()`
#' @param hist_plot_title Title of histogram plot
#' @param plot_vs_pe_column Statistic to plot versus the `point_estimate` (can only be either `mean` or `median`)
#' @param by_page_var Variable (age, sex, location, or year) that differentiates by page
#' @param facet_var Variable (age, sex, location, or year) to facet plots by
#' @param color_scatter_var Variable (age, sex, location, or year) to differentiate by color in each plot
#' @param output_path Full path to save the pdf of plots to (must have `.pdf` extension)
#' @param pdf_height Height of pdf
#' @param pdf_width Width of pdf
#' @param verbose `TRUE` or `FALSE` - should the function notify you the variables that are printing?
#'
#' @note Does not return anything, but saves pdf to `output_path`
#'
#' @export
plot_draw_pe_distribution <- function(
    DT
    , make_pdf               = TRUE
    , output_path
    , plot_vs_pe_column      = 'mean'
    , by_page_var            = 'year_id'
    , facet_var              = 'age_group_id'
    , color_scatter_var      = 'location_id'
    , hist_plot_title        = "Histogram"
    , pdf_height             = 10
    , pdf_width              = 20
    , verbose                = FALSE
    , plot_location_metadata = internal_metadata$location
    , plot_sex_metadata      = internal_metadata$sex
    , plot_age_metadata      = internal_metadata$age
) {

  plot_vars <- c('age_group_id', 'sex_id', 'location_id', 'year_id')
  checkmate::assert_data_table(DT)
  assert_x_in_y(plot_vs_pe_column, c('mean', 'median'))
  assert_x_in_y(c('point_estimate', plot_vs_pe_column, plot_vars), colnames(DT))
  checkmate::assert_subset(
    x        = by_page_var,
    choices  = plot_vars,
    empty.ok = TRUE
  )
  checkmate::assert_subset(
    x        = facet_var,
    choices  = plot_vars,
    empty.ok = TRUE
  )
  checkmate::assert_subset(
    x        = color_scatter_var,
    choices  = plot_vars,
    empty.ok = TRUE
  )
  checkmate::assert_flag(x = verbose, null.ok = FALSE)
  checkmate::assert_flag(x = make_pdf, null.ok = FALSE)


  if (make_pdf == TRUE) {

    checkmate::assert_subset(x = tools::file_ext(output_path), choices = 'pdf')
    checkmate::assert_directory_exists(x = dirname(output_path))
    checkmate::assert_integerish(
      x           = pdf_width,
      lower       = 1,
      upper       = 50,
      any.missing = FALSE
    )
    checkmate::assert_integerish(
      x           = pdf_height,
      lower       = 1,
      upper       = 50,
      any.missing = FALSE
    )
    checkmate::assert_string(x = hist_plot_title, null.ok = FALSE)
    pdf(
      file   = output_path,
      height = pdf_height,
      width  = pdf_width
    )

    hist_plot <- make_histogram(DT, hist_plot_title)
    plot(hist_plot)

    plot_scatters(
      DT
      , plot_vs_pe_column
      , by_page_var
      , facet_var
      , color_scatter_var
      , plot_vars
      , verbose
      , plot_location_metadata = plot_location_metadata
      , plot_sex_metadata      = plot_sex_metadata
      , plot_age_metadata      = plot_age_metadata
    )

    dev.off()

    message('pdf saved to: ', output_path)

  } else {

    return(list(
      histogram  = make_histogram(DT, hist_plot_title)
      , scatters = plot_scatters(
        DT
        , plot_vs_pe_column
        , by_page_var
        , facet_var
        , color_scatter_var
        , plot_vars
        , verbose
        , plot_location_metadata = plot_location_metadata
        , plot_sex_metadata      = plot_sex_metadata
        , plot_age_metadata      = plot_age_metadata
      )
    ))

  }
}

make_histogram <- function(DT, hist_plot_title) {
  ggplot2::ggplot(
    data = DT,
    mapping = ggplot2::aes(x = pe_percentile)
  ) +
    ggplot2::geom_histogram(binwidth=0.01, fill="blue", color="black", alpha=0.5) +
    ggplot2::labs(
      title = hist_plot_title,
      x = "Percentile of Point Estimate",
      y = "Frequency"
    ) +
    ggplot2::theme_minimal()
}

remap_vars <- function(var_name) {
  switch(
    var_name,
    'location_id'  = 'super_region_name',
    'age_group_id' = 'age_group_name',
    'sex_id'       = 'sex_name',
    'year_id'
  )
}

merge_ihme_metadata <- function(
    DT
    , plot_location_metadata = internal_metadata$location
    , plot_sex_metadata      = internal_metadata$sex
    , plot_age_metadata      = internal_metadata$age
) {
  if(!('super_region_name' %in% colnames(DT))) {
    DT <- data.table::merge.data.table(
      x  = DT
      , y  = plot_location_metadata
      , by = 'location_id'
      , all.x = TRUE
    )
  }
  if(!('sex_name' %in% colnames(DT))) {
    DT <- data.table::merge.data.table(
      x  = DT
      , y  = plot_sex_metadata
      , by = 'sex_id'
      , all.x = TRUE
    )
  }
  if('age_group_name' %in% colnames(DT)) DT$age_group_name <- NULL
  DT <- data.table::merge.data.table(
    x  = DT
    , y  = plot_age_metadata
    , by = 'age_group_id'
    , all.x = TRUE
  )
  return(DT)
}

plot_scatters <- function(
    DT
    , plot_vs_pe_column
    , by_page_var
    , facet_var
    , color_scatter_var
    , plot_vars
    , verbose
    , plot_location_metadata = internal_metadata$location
    , plot_sex_metadata      = internal_metadata$sex
    , plot_age_metadata      = internal_metadata$age
) {

  DT <- merge_ihme_metadata(
    DT
    , plot_location_metadata = plot_location_metadata
    , plot_sex_metadata      = plot_sex_metadata
    , plot_age_metadata      = plot_age_metadata
  )

  remaining_var <- setdiff(
    x = plot_vars,
    y = c(facet_var, by_page_var, color_scatter_var)
  ) |>
    remap_vars()

  facet_var         <- remap_vars(facet_var)
  by_page_var       <- remap_vars(by_page_var)
  color_scatter_var <- remap_vars(color_scatter_var)

  num_plot_cols_per_page <- ceiling(sqrt(length(unique(DT[[remaining_var]]))))

  for(a in sort(unique(DT[[by_page_var]]))) {
    plot_list    <- lapply(sort(unique(DT[[remaining_var]])), \(b) {
      i_vec      <- which(DT[[by_page_var]] == a & DT[[remaining_var]] == b)
      plot_dat   <- DT |> dplyr::slice(i_vec)
      plot_title <- paste(a, b, sep = ' - ')
      if(verbose) print(paste('plotting:', plot_title))

      p <- ggplot2::ggplot(
        data    = plot_dat,
        mapping = ggplot2::aes(x = get(plot_vs_pe_column), y = point_estimate)
      ) +
        ggplot2::geom_point(
          mapping  = ggplot2::aes(
            colour = get(color_scatter_var)
          )
        ) +
        ggplot2::geom_abline(slope = 1, colour = 'black', linetype = 'dashed') +
        ggplot2::labs(
          title  = plot_title,
          x      = plot_vs_pe_column,
          y      = 'point_estimate',
          colour = color_scatter_var
        ) +
        ggplot2::theme_minimal() +
        ggplot2::theme(
          axis.text.x = ggplot2::element_text(angle = 30, vjust = 0.95, hjust=1)
        )
      if (length(unique(DT[[facet_var]])) > 1) {
        p <- p +
          ggplot2::facet_wrap(~get(facet_var))
      }
      return(p)
    })
    do.call(
      # e.g. gridExtra::grid.arrange
      getExportedValue('gridExtra', 'grid.arrange'),
      c(plot_list, ncol = num_plot_cols_per_page)
    )
  }
}
